                      ...
                      <div className="flex items-center justify-between p-3 bg-accent/20 rounded-lg">
                        ...
                        <div className={`flex items-center gap-1 ${
                          ...
                          <span className="font-bold">
                            {isPositiveReturn ? '+' : ''}${(Number(portfolio.performance?.totalReturn) || 0).toLocaleString()}
                          </span>
                          <span className="text-sm">
                            ({isPositiveReturn ? '+' : ''}{(Number(portfolio.performance?.returnPercentage) || 0).toFixed(2)}%)
                          </span>
                        </div>
                      </div>
                    </CardContent>
                    ...
                    <CardContent className={`space-y-4 ${isMobile ? 'p-4 pt-0' : ''}`}>
                      ...
                      <div className={`grid ${isMobile ? 'grid-cols-1 gap-3' : 'grid-cols-2 gap-4'}`}>
                        <div>
                          ...
                          <p className="font-medium text-foreground">{portfolio.strategy?.timeHorizon || 'Not specified'}</p>
                        </div>
                        <div>
                          <p className="text-sm text-muted-foreground">Rebalance</p>
                          <p className="font-medium text-foreground">{portfolio.strategy?.rebalanceFrequency || 'Not specified'}</p>
                        </div>
                      </div>
                      <div>
                    ...
                    <CardContent className={isMobile ? 'p-4 pt-0' : ''}>
                      <div className={`space-y-3 ${isMobile ? 'max-h-40' : 'max-h-48'} overflow-y-auto scrollbar-default`}>
                        {(portfolio.allocations || []).map((allocation, index) => (
                          <div key={allocation?.symbol || index} className="flex items-center justify-between p-3 rounded-lg bg-accent/20">
