...
export function PortfolioSection() {
  ...
  // Load portfolios from the centralized portfolio system
  const loadPortfolios = () => {
    setIsLoading(true);
    console.log(`🔄 Loading portfolios... (refresh key: ${refreshKey})`);
    
    try {
      const portfolios = getPortfolios();
      console.log(`📊 Found ${portfolios.length} portfolios in localStorage`);
      
      if (portfolios.length === 0) {
        console.log('🔥 No portfolios found, trying to initialize default portfolios...');
        const initializedPortfolios = initializeDefaultPortfolios();
        console.log(`✅ Initialized ${initializedPortfolios.length} default portfolios`);
        
        if (initializedPortfolios.length > 0) {
          const convertedPortfolios = initializedPortfolios.map(convertToLocalPortfolio);
          setSavedWallets(convertedPortfolios);
          console.log('🎯 Converted and set portfolios:', convertedPortfolios.map(p => p.name));
        }
      } else {
        // Convert to local format
        const convertedPortfolios = portfolios.map(convertToLocalPortfolio);
        setSavedWallets(convertedPortfolios);
        console.log('✅ Loaded existing portfolios:', portfolios.map(p => p.name));
      }
    } catch (error) {
  ...
  const handleEnsureDefaultPortfolios = () => {
    setIsLoading(true);
    try {
      console.log('🔧 Ensuring all default portfolios exist...');
      const portfolios = ensureAllDefaultPortfolios();
      const convertedPortfolios = portfolios.map(convertToLocalPortfolio);
      setSavedWallets(convertedPortfolios);
      
      toast.success("Default portfolios restored!", {
        description: `Now showing ${portfolios.length} portfolios including all defaults.`,
        duration: 4000,
      });
    } catch (error) {
      console.error('Error ensuring default portfolios:', error);
      toast.error('Failed to restore default portfolios');
    } finally {
      setIsLoading(false);
    }
  };
              ...
              <p className="text-muted-foreground">
                {isLoading 
                  ...
                  : savedWallets.length === 0 
                    ? "No portfolios found. Try refreshing or restoring defaults." 
                    : `Manage and track your ${savedWallets.length} investment portfolios`
                }
              </p>
            </div>
            <div className="flex items-center gap-3">
              ...
              {savedWallets.length === 0 && (
                <Button
                  onClick={handleEnsureDefaultPortfolios}
                  variant="outline"
                  className="hover:bg-accent/50"
                  disabled={isLoading}
                >
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredWallets.map((wallet, index) => {
                ...
                const isDefaultPortfolio = wallet.id.startsWith('default-');
