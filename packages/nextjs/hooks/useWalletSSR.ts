...
export function useWalletSSRData(): UseWalletSSRData {
  ...
  const totalValue = portfolioTokens?.reduce((total, token) => total + (token.balanceUSD || 0), 0) || 0;
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      ...
      setWalletData(mockData as any);
      const tokens: PortfolioToken[] = Object.entries(mockData.tokens).map((entry, index) => {
        ...
        return {
          ...
          balanceUSD: parseFloat(balance as string) * [3000, 1, 60000][index] || 0,
