...
class ApiService {
  ...
  async getTokenChart(symbol: string, timeframe: "1h" | "24h" | "7d" | "30d" | "1y" = "24h"): Promise<ChartResponse> {
  ...
  async getWalletBalances(walletAddress: string, chainId: number = 1): Promise<ApiResponse<WalletBalances>> {
  ...
  private generateMockTokens(query: string, chainId: number): TokenDto[] {
    ...
    return mockTokens
      .filter(symbol => symbol.toLowerCase().includes(query.toLowerCase()) || query.length === 0)
      .slice(0, 10)
      .map((symbol, index) => ({
        chainId,
        symbol,
  ...
  private generateMockWalletBalances(walletAddress: string, chainId: number): WalletBalances {
    ...
    return {
      ...
      totalUSD: Object.values(tokens).reduce(
        (sum, balance) => sum + parseFloat(balance) * (Math.random() * 2000 + 100),
        0,
      ),
      lastUpdated: Date.now(),
    };
  ...
  private generateMockSwapQuote(fromToken: string, toToken: string, amount: string, chainId: number): SwapQuote {
